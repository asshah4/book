...
Here is an issue with a function using _for loops_. Generally, if there is an iterative process internal to the function, it will have an $~O(N^2)$ performance. The `card::recur_survival_table` is built with intuitive _for loops_, at a significant cost with increased _N_ sizes.
...
```{r}
print(card::recur_survival_table)
```
